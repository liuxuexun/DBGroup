...
parser.add_argument('--scannet_path', required=True, help='path to scannet data')  # The path include .sens file
parser.add_argument('--output_path', default='../../data/scannet_2d', help='where to output 2d data')
...
def main():
    ...
    scenes = sorted([d[:-5] for d in os.listdir(opt.scannet_path)])
    print('Found %d scenes' % len(scenes))
    for i in range(0,len(scenes)):
        sens_file = os.path.join('/data/share/scannet/scannet_video', scenes[i] + '.sens')
