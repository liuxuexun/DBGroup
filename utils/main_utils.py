...
def transform_pt_depth_scannet_torch(points, depth_intrinsic, depth, pose, device, is_s3dis=False):
    """
...
def isolate_on_pred(xyz, pt_pred, pt_score):
    ...
    for ins_id in ins_preds:            # 遍历每一个预测实例
        if ins_id == -1:
            continue
        ...
        if label_counts.shape[0] > 1:                   # 保留这个prompt点的最大的一簇点
